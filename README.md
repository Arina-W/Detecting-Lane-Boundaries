 ...
 * Below is the result of calibration done on a test image from the video.
 ...
 * In this step `cv2.Sobel` operator in x the direction are used since it does a clenaer job to pick up the lane lines.
```
